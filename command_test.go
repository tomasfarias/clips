...
func TestParseCommand(t *testing.T) {
	...
	if result.SubCommand != "" {
		t.Errorf("SubCommand not properly parsed: expected \"\" got %s", result.SubCommand)
	}
...
func TestParseCommandValidEmpty(t *testing.T) {
	inputCommand := "!clips"
	result, err := ParseCommand(inputCommand)
	if err != nil {
		t.Errorf("Got an error while parsing test command: %s", err)
...
func TestParseCommandSubCommandHelp(t *testing.T) {
	inputCommand := "!clips help"
	result, err := ParseCommand(inputCommand)
	if err != nil {
		t.Errorf("Got an error while parsing test command: %s", err)
	}
...
func TestParseCommandSubCommandTopEmpty(t *testing.T) {
	inputCommand := "!clips top"
	result, err := ParseCommand(inputCommand)
	if err != nil {
		t.Errorf("Got an error while parsing test command: %s", err)
	}
	...
	if result.Broadcaster != "" {
		t.Errorf("Broadcaster not properly parsed: expected \"\" got %s", result.Broadcaster)
	}
	if result.SubCommand != "top" {
		t.Errorf("SubCommand not properly parsed: expected \"\" got %s", result.SubCommand)
	}
	if result.Top != 10 {
		t.Errorf("Top not properly parsed: expected 10 got %d", result.Top)
	}
	if result.Title != "" {
...
func TestParseCommandSubCommandTop(t *testing.T) {
	inputCommand := "!clips top5 Streamer 7d"
	result, err := ParseCommand(inputCommand)
	if err != nil {
		t.Errorf("Got an error while parsing test command: %s", err)
	}
	...
	if result.SubCommand != "top" {
		t.Errorf("SubCommand not properly parsed: expected \"top\" got %s", result.SubCommand)
	}
	if result.Top != 5 {
		t.Errorf("Top not properly parsed: expected 5 got %d", result.Top)
	}
	if result.Title != "" {
		t.Errorf("Title not properly parsed: expected \"\" got %s", result.Title)
	}
	...
	now := time.Now()
	currentDate := time.Date(now.Year(), now.Month(), now.Day(), now.Hour(), now.Minute(), 0, 0, now.Location())
